...
const MultipleQRGenerator = () => {
  ...
  const addAppliance = async () => {
    // Only name and modelNo are required
    if (!formData.name || !formData.modelNo) {
      setError('Name and Model No are required');
      return;
    }
    ...
    try {
      ...
      setFormData({ name: '', modelNo: '', purchaseDate: '', serviceDate: '' });
    } catch (err) {
  ...
  const generatePDF = async () => {
    ...
    // Calculate positions for grid layout (10 columns x 4 rows = 40 per page)
    const cols = 12; // Increased columns for landscape mode
    const rows = 7; // Reduced rows for landscape mode

    for (let i = 0; i < appliances.length; i++) {
      if (i > 0 && i % (cols * rows) === 0) {
        doc.addPage(); // Add a new page after every 40 QR codes
      }
      ...
      if (qrCodeElement) {
        ...
        doc.setFontSize(8);
        doc.text(
          appliance.name.substring(0, 15), // Truncate long names
          x + qrSize / 2,
          y + qrSize + 3,
          { align: 'center' }
        );
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      {appliances.length > 0 && (
        <div className="mt-6">
          ...
          <div className="grid grid-cols-10 gap-1"> {/* Adjusted grid for landscape preview */}
          ...
          >
            Export All as PDF (40 per page, Landscape)
          </button>
        </div>
      )}
    </div>
